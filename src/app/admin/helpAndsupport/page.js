...
const initialTickets = {
  candidates: [
    {
      id: 'SUP-4201', name: 'John Carter', img: 'avata1', issue: 'Resume upload failed', category: 'Profile', adminStatus: 'Pending', userStatus: 'Pending', date: '14 Apr 2026',
    ...
    {
      id: 'SUP-4202', name: 'Aisha Khan', img: 'avata2', issue: 'Payment issue', category: 'Billing', adminStatus: 'Resolved', userStatus: 'Resolved', date: '12 Apr 2026',
  ...
  recruiters: [
    {
      id: 'SUP-5101', name: 'TechFlow Pvt Ltd', img: 'avata3', issue: 'Job posting error', category: 'Technical', adminStatus: 'Pending', userStatus: 'Pending', date: '15 Apr 2026',
    ...
    {
      id: 'SUP-5102', name: 'NextHire Ltd', img: 'avata4', issue: 'Invoice mismatch', category: 'Billing', adminStatus: 'In Progress', userStatus: 'Pending', date: '13 Apr 2026',
...
export default function AdminSupportPage() {
  ...
  const filteredData = data.filter((ticket) => (
    ...
    (!status || ticket.adminStatus === status) && (!category || ticket.category === category)
  ))
  ...
  const sendReply = () => {
    ...
    // Replying only ever affects the admin-side status, never the candidate/recruiter status.
    updateTicket(selectedTicket.id, { messages: [...selectedTicket.messages, message], adminStatus: selectedTicket.adminStatus === 'Pending' ? 'In Progress' : selectedTicket.adminStatus })
    setReply('')
  }

  // Admin changes their own status only. The candidate/recruiter status is left untouched here —
  // it only changes when the candidate/recruiter resolves it from their own panel (see resolveByUser below).
  const changeStatus = (nextStatus) => selectedTicket && updateTicket(selectedTicket.id, { adminStatus: nextStatus })

  // Called from the candidate/recruiter panel when the ticket owner marks their own ticket resolved.
  // In that case BOTH sides flip to Resolved together.
  const resolveByUser = (id) => updateTicket(id, { adminStatus: 'Resolved', userStatus: 'Resolved' })
    ...
    <>
        ...
        <div className="panel-white support-panel">
          ...
          <div className="box-padding support-table-wrap">
            <table className="support-table">
              <thead><tr><th>User</th><th>Subject / Issue</th><th>Category</th><th>Raised on</th><th>{activeTab === 'candidates' ? 'Candidate' : 'Recruiter'} Status</th><th>Admin Status</th><th className="action-column">Actions</th></tr></thead>
              <tbody>
                {filteredData.map((ticket) => <tr key={ticket.id}>
                  ...
                  <td><span className="ticket-status" style={statusStyle[ticket.userStatus]}>{ticket.userStatus}</span></td>
                  <td><span className="ticket-status" style={statusStyle[ticket.adminStatus]}>{ticket.adminStatus}</span></td>
      ...
      {selectedTicket && <>
        ...
        <aside className="support-drawer" aria-label="Ticket details">
          ...
          <div className="ticket-summary"><div className="ticket-user"><img src={`/assets/imgs/page/dashboard/${selectedTicket.img}.png`} alt="" /><div><h6>{selectedTicket.name}</h6><span>{activeTab === 'candidates' ? 'Candidate' : 'Recruiter'} · {selectedTicket.category}</span></div></div><div className="ticket-status-group"><span className="status-pair-label">{activeTab === 'candidates' ? 'Candidate' : 'Recruiter'}<span className="ticket-status" style={statusStyle[selectedTicket.userStatus]}>{selectedTicket.userStatus}</span></span><span className="status-pair-label">Admin<span className="ticket-status" style={statusStyle[selectedTicket.adminStatus]}>{selectedTicket.adminStatus}</span></span></div></div>
          ...
          {selectedTicket.adminStatus !== 'Resolved' ? <div className="reply-area"><label htmlFor="admin-reply">Reply to {selectedTicket.name}</label><div><textarea id="admin-reply" value={reply} onChange={(event) => setReply(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') sendReply() }} placeholder="Write a helpful reply..." rows="3" /><button type="button" className="btn btn-default reply-send" onClick={sendReply} disabled={!reply.trim()}><Send size={16} /> Send Reply</button></div><small>Press Ctrl/Cmd + Enter to send</small></div> : <div className="resolved-note"><CheckCircle2 size={18} /> You have marked this ticket as resolved. Reopen it if more help is needed.</div>}
          {selectedTicket.userStatus === 'Resolved' && selectedTicket.adminStatus !== 'Resolved' && <div className="resolved-note user-resolved-note"><CheckCircle2 size={18} /> The {activeTab === 'candidates' ? 'candidate' : 'recruiter'} has marked this ticket as resolved on their end.</div>}
          <div className="drawer-actions">{selectedTicket.adminStatus !== 'In Progress' && selectedTicket.adminStatus !== 'Resolved' && <button type="button" className="btn btn-grey-small progress-action" onClick={() => changeStatus('In Progress')}><Clock3 size={16} /> Mark In Progress</button>}{selectedTicket.adminStatus !== 'Resolved' ? <button type="button" className="btn btn-default resolve-action" onClick={() => changeStatus('Resolved')}><CheckCircle2 size={16} /> Mark Resolved</button> : <button type="button" className="btn btn-grey-small progress-action" onClick={() => changeStatus('In Progress')}><Clock3 size={16} /> Reopen Ticket</button>}</div>
        </aside>
      </>}
