...
const normalizeCountryData = (countriesList) => {
  const merged = {};
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    ...
    const loadRevenueOverview = async () => {
      ...
      try {
        ...
        setCountryData(normalizeCountryData(byCountryData.countries || []));
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      ...
      try {
        ...
        if (dateFrom) {
          ...
          query.append('dateFrom', dateFrom);
          query.append('StartDate', dateFrom);
          query.append('startDate', dateFrom);
        }
        if (dateTo) {
          query.append('DateTo', dateTo);
          query.append('dateTo', dateTo);
          query.append('EndDate', dateTo);
          query.append('endDate', dateTo);
        }

        const response = await apiRequest(`/api/admin/revenue/transactions?${query.toString()}`, { method: 'GET' });
        if (!active) return;
    ...
    loadTransactions();
    return () => { active = false; };
  }, [page, search, country, transactionType, dateFrom, dateTo]);
  ...
  const pageCount = Math.max(1, transactionMeta.totalPages);
