...
const formatTransactionDate = (value) => {
  ...
  return `${String(date.getDate()).padStart(2, '0')} ${monthShort[date.getMonth()]} ${date.getFullYear()}`;
};
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    ...
    const loadRevenueOverview = async () => {
      ...
      setError('');
      try {
        const [summaryResponse, countryResponse] = await Promise.all([
          apiRequest('/api/admin/revenue/summary', { method: 'GET' }),
          apiRequest('/api/admin/revenue/by-country', { method: 'GET' }),
        ]);
        if (!active) return;
        const summaryData = unwrapApiData(summaryResponse);
        const byCountryData = unwrapApiData(countryResponse);
        setSummary(summaryData);
        setCountryData(byCountryData.countries || []);
        setComposition(byCountryData.composition || { candidatePercent: 0, recruiterPercent: 0, creditsPercent: 0 });
      } catch (requestError) {
        if (active) setError(requestError.message || 'Failed to load revenue data.');
      } finally {
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      ...
      try {
        const response = await apiRequest(`/api/admin/revenue/transactions?page=${page}&pageSize=10`, { method: 'GET' });
        if (!active) return;
        const data = unwrapApiData(response);
        setApiTransactions((data.items || []).map(mapTransaction));
        setTransactionMeta({ totalCount: data.totalCount || 0, totalPages: data.totalPages || 1 });
      } catch (requestError) {
        if (active) setError(requestError.message || 'Failed to load revenue transactions.');
      } finally {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return apiTransactions.filter((item) => {
  ...
  }, [apiTransactions, transactionType, country, search, dateFrom, dateTo]);
  const hasLocalFilters = transactionType !== 'all' || country !== 'All countries' || Boolean(search || dateFrom || dateTo);
  const pageCount = hasLocalFilters ? 1 : Math.max(1, transactionMeta.totalPages);
  ...
  const openInvoice = async (item) => {
    try {
      ...
      setInvoice({ ...item, ...mapTransaction(unwrapApiData(response)) });
    } catch (requestError) {
      setError(requestError.message || 'Failed to load invoice.');
    }
  };
  const downloadInvoice = async () => {
    if (!invoice?.id) return;
    try {
      const pdf = await apiRequest(`/api/admin/revenue/transactions/${invoice.id}/invoice/download`, {
        method: 'GET',
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.download = `${invoice.invoice || 'invoice'}.pdf`;
      link.click();
    ...
    <>
      <div className="revenue-page">
        ...
        <section className="revenue-filter-bar">
          <div className="filter-title"><span style={{ fontSize: "16px" }}>Revenue reporting</span><small>Filter totals and transactions by market</small></div>
        ...
        <div className="revenue-insights">
          ...
          <aside className="period-panel"><span>{timePeriod === 'monthly' ? 'Monthly revenue' : 'Yearly revenue'}</span><h3>₹{loading ? '—' : totals.toLocaleString()}</h3><p>Across {country === 'All countries' ? 'all active countries' : country}</p><div className="revenue-composition"><div><i className="candidate" />Candidate memberships <b>{Number(composition.candidatePercent || 0).toFixed(1)}%</b></div><div><i className="recruiter" />Recruiter memberships <b>{Number(composition.recruiterPercent || 0).toFixed(1)}%</b></div><div><i className="credits" />Credit plans <b>{Number(composition.creditsPercent || 0).toFixed(1)}%</b></div></div></aside>
        </div>
        ...
        <section className="transactions-panel">
          ...
          {visibleTransactions.length > 0 && <div className="table-pagination"><span>Showing {(page - 1) * 10 + 1}–{(page - 1) * 10 + visibleTransactions.length} of {hasLocalFilters ? visibleTransactions.length : transactionMeta.totalCount} transactions</span><div><button disabled={page === 1 || hasLocalFilters} onClick={() => setPage((current) => current - 1)}>Previous</button>{Array.from({ length: pageCount }, (_, index) => <button key={index} className={page === index + 1 ? 'active' : ''} onClick={() => setPage(index + 1)}>{index + 1}</button>)}<button disabled={page === pageCount || hasLocalFilters} onClick={() => setPage((current) => current + 1)}>Next</button></div></div>}
        ...
        <style jsx>{`
          .revenue-page { color: #172b60; padding-bottom: 35px; } .revenue-heading { margin-bottom: 23px; } .revenue-error { margin: 14px 0 -5px; padding: 10px 14px; border: 1px solid #f3b7b7; border-radius: 8px; background: #fff1f1; color: #b42525; font-size: 12px; font-weight: 700; }
