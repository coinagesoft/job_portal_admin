...
import { BriefcaseBusiness, Calendar, ChartNoAxesCombined, Coins, Download, FileText, Search, Users, X } from 'lucide-react';
...
const transactions = ["candidate", "recruiter", "credits"].flatMap(
  (type, typeIndex) =>
    Array.from({ length: 10 }, (_, index) => {
      const [country, code] =
        countries[(index + typeIndex * 4) % countries.length];
      ...
      return {
        id: `REV-${number}`,
        date: dates[index],
        customer: customerNames[(index + typeIndex * 3) % customerNames.length],
        plan:
          type === "candidate"
            ? "Candidate Lifetime"
            : type === "recruiter"
...
const countryRevenue = [
  ...
  { country: "Kuwait", code: "KWD", monthly: 9610, yearly: 107340 },
  { country: "Bahrain", code: "BHD", monthly: 8340, yearly: 93120 },
  { country: "Oman", code: "OMR", monthly: 7780, yearly: 87020 },
  { country: "United Kingdom", code: "GBP", monthly: 7540, yearly: 84360 },
  { country: "Australia", code: "AUD", monthly: 6840, yearly: 76480 },
  { country: "Egypt", code: "EGP", monthly: 5910, yearly: 66080 },
  { country: "Jordan", code: "JOD", monthly: 4890, yearly: 54630 },
  { country: "Lebanon", code: "LBP", monthly: 3570, yearly: 39870 },
  { country: "Turkey", code: "TRY", monthly: 3180, yearly: 35490 },
];

const typeLabels = {
  all: "All transactions",
  candidate: "Candidate memberships",
  recruiter: "Recruiter memberships",
  credits: "Credit plans",
};
const typeStyles = {
  candidate: "candidate",
  recruiter: "recruiter",
...
export default function RevenuePage() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((item) => {
      ...
      return (
        ...
        (!to || (txDate && txDate <= to)) &&
        `${item.id} ${item.customer} ${item.invoice}`
          .toLowerCase()
  ...
  const changeDateFrom = (value) => { setDateFrom(value); setPage(1); };
  const changeDateTo = (value) => { setDateTo(value); setPage(1); };
  ...
  const cards = [
    ...
    },
    {
      ...
      amount: "$18,820",
      detail: "21.0% of revenue",
      icon: Users,
      color: "mint",
    },
    {
      title: "Recruiter memberships",
      amount: "$46,920",
      detail: "52.5% of revenue",
      icon: BriefcaseBusiness,
      color: "blue",
    },
    {
      title: "Recruiter credit plans",
      amount: "$23,670",
      detail: "26.5% of revenue",
      icon: Coins,
      color: "purple",
    },
  ];

  const totals = countryRevenue.reduce(
    (sum, item) => sum + item[timePeriod],
    0,
  );
    ...
    <div className="revenue-page">
      <div className="box-heading revenue-heading">
        <div className="box-title">
          <h3 className="mb-5">Revenue</h3>
          <p className="font-sm color-text-paragraph-2 mb-0">
            Track membership and credit plan revenue across every market.
          </p>
        </div>
      ...
      <section className="revenue-filter-bar">
        <div className="filter-title">
          ...
          <small>Filter totals and transactions by market</small>
        </div>
        <div className="report-filters">
          ...
          <div className="date-range" aria-label="Custom date range">
            ...
            <input type="date" value={dateFrom} onChange={(event) => changeDateFrom(event.target.value)} max={dateTo || undefined} aria-label="From date" />
            <span className="date-range-sep">–</span>
            <span className="date-range-label">To</span>
            <input type="date" value={dateTo} onChange={(event) => changeDateTo(event.target.value)} min={dateFrom || undefined} aria-label="To date" />
        ...
        {cards.map((card) => {
          ...
          return (
            <article className="revenue-card" key={card.title}>
              <span className={`revenue-icon ${card.color}`}>
                <Icon size={21} />
              </span>
              <div>
                <p>{card.title}</p>
      ...
      <div className="revenue-insights">
        <section className="country-panel">
          <div className="panel-heading">
            <div>
              ...
              <p>
                {timePeriod === "monthly" ? "Current month" : "Current year"}{" "}
                revenue performance
              </p>
            </div>
            ...
            {countryRevenue
              ...
              .map((item) => (
                <div className="country-row" key={item.country}>
                  <div>
                    <span className="country-dot">{item.code.slice(0, 2)}</span>
                    <b>{item.country}</b>
                  </div>
                  <div className="country-progress">
                    <i
                      style={{
                        width: `${Math.round((item[timePeriod] / totals) * 100)}%`,
                      }}
                    />
                  </div>
                  <strong>${item[timePeriod].toLocaleString()}</strong>
                </div>
              ))}
          </div>
        </section>
        <aside className="period-panel">
          <span>
            {timePeriod === "monthly" ? "Monthly revenue" : "Yearly revenue"}
          </span>
          <h3>${totals.toLocaleString()}</h3>
          <p>
            Across{" "}
            {country === "All countries" ? "all active countries" : country}
          </p>
          <div className="revenue-composition">
            <div>
              <i className="candidate" />
              Candidate memberships <b>21%</b>
            </div>
            <div>
              <i className="recruiter" />
              Recruiter memberships <b>52.5%</b>
            </div>
            <div>
              <i className="credits" />
              Credit plans <b>26.5%</b>
            </div>
          </div>
        </aside>
      </div>
      ...
      <section className="transactions-panel">
        <div className="transactions-top">
          <div>
            ...
            <p>
              {dateFrom || dateTo
                ? `Showing transactions ${dateFrom ? `from ${dateFrom}` : "up to"}${dateFrom && dateTo ? " to " : ""}${dateTo ? dateTo : dateFrom ? " onward" : ""}.`
        ...
        <div className="transaction-tabs">
          {Object.entries(typeLabels).map(([id, label]) => (
            <button
              key={id}
              className={transactionType === id ? "active" : ""}
            ...
            <tbody>
              {visibleTransactions.map((item) => (
                <tr key={item.id}>
                  <td>{item.customer}</td>
                  <td>
                    <span className={`plan-type ${typeStyles[item.type]}`}>
                      {item.plan}
                    </span>
                  </td>
                  <td>
        ...
        {filteredTransactions.length > 0 && (
          <div className="table-pagination">
            <span>
              Showing {(page - 1) * 10 + 1}–
              {Math.min(page * 10, filteredTransactions.length)} of{" "}
      ...
      {invoice && (
        <div className="invoice-overlay" role="dialog" aria-modal="true">
          <div className="invoice-modal">
            ...
            <span className="invoice-logo">JOBBOX</span>
            <h3>Invoice {invoice.invoice}</h3>
            <p>
              Payment received from <b>{invoice.customer}</b>
            </p>
            <div className="invoice-total">
              <span>{invoice.plan}</span>
              <b>${invoice.amount.toLocaleString()}</b>
            </div>
            <div className="invoice-meta">
              <span>Date: {invoice.date}</span>
              <span>Country: {invoice.country}</span>
              <span>Payment: {invoice.method}</span>
            </div>
            <button className="download-invoice" onClick={() => window.print()}>
              <Download size={16} />
              Download invoice
            </button>
          </div>
      ...
      <style jsx>{`
        ...
        .revenue-filter-bar { padding: 17px 22px; display: flex; align-items: center; justify-content: space-between; gap: 18px; border-radius: 12px; background: #fff; border: 1px solid #e3eaf4; box-shadow: 0 7px 20px rgba(34,59,115,.04); } .filter-title { display: grid; gap: 3px; } .filter-title span { font-size: 14px; font-weight: 800; } .filter-title small, .panel-heading p, .transactions-top p { color: #7382a1; font-size: 12px; margin: 0; } .report-filters { display: flex; align-items: center; gap: 11px; } .report-filters select { min-width: 190px; height: 39px; border: 1px solid #d6e0ee; background: #fff; border-radius: 7px; color: #344976; padding: 0 10px; font-size: 12px; font-weight: 700; outline: none; } .date-range { display: flex; align-items: center; gap: 8px; height: 39px; padding: 0 12px; border: 1px solid #d6e0ee; border-radius: 7px; background: #fff; box-sizing: border-box; flex-shrink: 0; } .date-range-icon { color: #8592aa; flex-shrink: 0; } .date-range-label { color: #8592aa; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: .03em; flex-shrink: 0; } .date-range input[type="date"] { -webkit-appearance: none; appearance: none; box-sizing: border-box; border: 0; outline: 0; background: transparent; color: #344976; font-size: 12px; font-weight: 600; font-family: inherit; line-height: normal; padding: 0; margin: 0; height: 100%; width: 128px; min-width: 128px; } .date-range input[type="date"]::-webkit-datetime-edit { padding: 0; } .date-range input[type="date"]::-webkit-datetime-edit-fields-wrapper { padding: 0; } .date-range input[type="date"]::-webkit-inner-spin-button { display: none; } .date-range input[type="date"]::-webkit-calendar-picker-indicator { width: 14px; height: 14px; padding: 0; margin-left: 5px; opacity: .55; cursor: pointer; } .date-range-sep { color: #b7c1d6; font-size: 12px; } .date-range-clear { display: grid; place-items: center; width: 22px; height: 22px; margin-left: 2px; border: 0; border-radius: 50%; background: #f3f6fb; color: #7382a1; cursor: pointer; flex-shrink: 0; } .date-range-clear:hover { background: #ffe9e9; color: #c62828; } .period-switch { padding: 3px; border-radius: 7px; background: #f2f5fa; display: flex; } .period-switch button { border: 0; background: transparent; padding: 7px 11px; color: #7786a4; border-radius: 5px; font-size: 12px; font-weight: 700; } .period-switch button.active { background: #fff; color: #ef9600; box-shadow: 0 2px 5px rgba(35,59,106,.1); }
